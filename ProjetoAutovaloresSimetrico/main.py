...
def calcular_multiplicidades(A, autovalores, tol=1e-4):
    ...
    for val in unicos:
        #Multiplicidade algébrica (n vezes que o autovalor aparece)
        ma = np.sum(valores_arredondados == val)
        
        #Multiplicidade geométrica (n - colunas_li) PESADO!
        if calcular_mg:
            I = np.eye(n, dtype=np.float64)
            colunas_li = np.linalg.matrix_rank(A - val * I, tol=tol)
            mg = n - colunas_li
        else:
            mg = "N/A"
...
def rodar_teste(A, nome_teste, fase):
    ...
    if fase == 4:
        print(f"[{nome_teste}] - Iniciando Fase 4 (Matriz da Equipe)...")
    else:
    ...
    if n == 5 and fase == 3:
        ...
        print(f"1. Adoção da Forma de Hessenberg : {'[V] COMPROVADO' if is_hessenberg else '[X] FALHOU'}")
        print(f"2. Colapso Tridiagonal           : {'[V] COMPROVADO' if is_tridiagonal else '[X] FALHOU'}")
        print(f"3. Preservação da Simetria       : {'[V] COMPROVADO' if is_simetrica else '[X] FALHOU'}")
        print("-" * 70)
    elif n <= 5:
    ...
    if n <= 5:
        ...
        pasta_resultados = f"resultados_fase{fase}"
...
def menu_principal():
    while True:
        ...
        print("[4] FASE 4 - Matriz 100x100 da Equipe")
        ...
        if escolha == '0':
            ...
            rodar_teste(A, "Matriz_4x4", fase=3)
            
        elif escolha == '2':
            try:
                n = int(input("Digite o tamanho desejado para a matriz (N): "))
                if n <= 0:
                    print("[!] Erro: O tamanho da matriz deve ser um inteiro positivo maior que zero.")
                    continue
                
                A = gerar_matriz_generica_real(n)
                rodar_teste(A, f"Matriz_{n}x{n}", fase=3)
